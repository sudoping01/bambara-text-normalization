...
class TestTimeNormalization:
    def test_time_to_bambara_whole_hour(self):
        result = time_to_bambara(1, 0)
        assert result == "Nɛgɛ kaɲɛ kelen"

    def test_time_to_bambara_with_minutes(self):
    ...
    def test_normalizer_with_expand_times(self):
        result = normalize("A nana 7:30 la", expand_times=True)
        assert "nɛgɛ kaɲɛ" in result
        assert "7:30" not in result

    def test_normalizer_without_expand_times(self):
    ...
    def test_round_trip_clock_time(self):
        original = time(7, 30)
    ...
    def test_time_to_bambara_midnight(self):
        result = time_to_bambara(0, 0)
        assert "fu" in result  # 0

    def test_time_to_bambara_noon(self):
        result = time_to_bambara(12, 0)
        assert "tan ni fila" in result  # 12
